// Publisher Dashboard — final static version
...
async function loadData() {
    ...
    try {
        ...
        document.getElementById('summary-cards').innerHTML = '<div class="col-span-2 lg:col-span-4 card p-8 text-center"><p class="text-gray-500">No data available yet. Data will appear once your sites start receiving traffic.</p></div>';
    }
}
...
function renderData() {
    ...
    const generated = new Date(publisherData.generated_at).toLocaleString('en-US', { month:'short', day:'numeric', year:'numeric', hour:'2-digit', minute:'2-digit' });
    document.getElementById('last-updated').textContent = `Last updated: ${generated}`;
    const totals = publisherData.totals[currentPeriod];
    const cards = [
        { label:'Impressions', value:formatNumber(totals.impressions), icon:'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z', bg:'bg-blue-500/10', text:'text-blue-400' },
        { label:'Revenue', value:formatCurrency(totals.revenue), icon:'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1', bg:'bg-green-500/10', text:'text-green-400' },
        { label:'eCPM', value:formatCurrency(totals.ecpm), icon:'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', bg:'bg-purple-500/10', text:'text-purple-400' },
        { label:'Clicks', value:formatNumber(totals.clicks), icon:'M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5', bg:'bg-orange-500/10', text:'text-orange-400' },
    ];
    ...
    if (!publisherData.sites || publisherData.sites.length === 0) { tbody.innerHTML = '<tr><td colspan="6" class="px-6 py-12 text-center text-gray-500">No sites configured yet</td></tr>'; return; }
    tbody.innerHTML = publisherData.sites.map(s => {
        const p = s.periods[currentPeriod];
        return `<tr class="hover:bg-white/5 transition-colors">
            <td class="px-6 py-4 text-sm font-semibold text-white">${s.site_name}</td>
...
function renderChart() {
    ...
    const cd = publisherData.daily_chart;
